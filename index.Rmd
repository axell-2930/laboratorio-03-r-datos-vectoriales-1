...
kable(world[1:10, 1:5, drop = TRUE],caption="Datos demográficos de los países del mundo")
...
kable(world[1:10, c("name_long","continent","pop","lifeExp","gdpPercap"), drop = TRUE],col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),caption="Datos demográficos de los países del mundo")
...
kable(world.ordenadoxpais[, c("name_long","continent","pop","lifeExp","gdpPercap"), drop = TRUE],col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),caption="Datos demográficos de los países del mundo") %>% kable_styling() %>% scroll_box(width = "800px",height = "500px",fixed_thead = TRUE)
...
centro_america = world[world$name_long == "Belize" | world$name_long == "Guatemala" | world$name_long == "Honduras" | world$name_long == "El Salvador" | world$name_long == "Nicaragua" | world$name_long == "Costa Rica" | world$name_long == "Panama", ]

kable(centro_america[, c("name_long","continent","pop","lifeExp","gdpPercap"), drop = TRUE],col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),caption="Datos demográficos de los países de Centroamérica") %>%kable_styling()
...
ggplot(centro_america, aes(x=reorder(name_long,lifeExp), y=lifeExp)) + ggtitle("Esperanza de vida de los paises Centroamericanos") + xlab("Porcentaje de personas") + ylab("Años") + geom_col() + geom_text(aes(label=lifeExp), vjust=-0.3, size=3.5)
...
```{r}
plot(centro_america["lifeExp"],main="Esperanza de vida en los paises centroamericano")
```
...
mayor_o_igual_74_anos = world[ world$name_long == "Nicaragua" | world$name_long == "Costa Rica" | world$name_long == "Panama", ]


kable(mayor_o_igual_74_anos[, c("name_long","lifeExp"), drop = TRUE],col.names=c("País","Esperanza de vida"),caption="Datos demográficos de los países de Centroamérica") %>%kable_styling()

plot(mayor_o_igual_74_anos["lifeExp"],main="Esperanza de vida en personas mayores o iguales a 74 años")
